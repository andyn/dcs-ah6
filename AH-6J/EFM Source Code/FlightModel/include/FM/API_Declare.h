...
extern "C" 
{
	...
	body coordinate system system is always X - positive forward ,
											...
											Z - positive to right 
	*/
	FM_API void ed_fm_add_local_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	FM_API void ed_fm_add_global_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	...
	*/
	FM_API bool ed_fm_add_local_force_component (double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	FM_API bool ed_fm_add_global_force_component (double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);

	FM_API void ed_fm_add_local_moment(double & x,double &y,double &z);
	FM_API void ed_fm_add_global_moment(double & x,double &y,double &z);
	...
	*/
	FM_API bool ed_fm_add_local_moment_component (double & x,double &y,double &z);
	FM_API bool ed_fm_add_global_moment_component (double & x,double &y,double &z);
	...
	*/
	FM_API void ed_fm_simulate(double dt);

	/*
	called before simulation to set up your environment for the next step
	give parameters of surface under your aircraft usefull for ground effect
	*/
	FM_API void ed_fm_set_surface (double		h,//surface height under the center of aircraft
	...
	called before simulation to set up your environment for the next step
	*/
	FM_API void ed_fm_set_atmosphere(double h,//altitude above sea level
	...
	called before simulation to set up your environment for the next step
	*/
	FM_API void ed_fm_set_current_mass_state (double mass,
	...
	called before simulation to set up your environment for the next step
	*/
	FM_API void ed_fm_set_current_state (double ax,//linear acceleration component in world coordinate system
	...
	*/
	FM_API void ed_fm_set_current_state_body_axis(double ax,//linear acceleration component in body coordinate system
	...
	input handling
	*/
	FM_API void ed_fm_set_command (int command,
							 float value);
	/*
	 Mass handling 
	...
	*/
	FM_API bool ed_fm_change_mass  (double & delta_mass,
	...
	*/
	FM_API void ed_fm_set_internal_fuel(double fuel);

	/*
		get internal fuel volume 
	*/
	FM_API double ed_fm_get_internal_fuel();

	/*
		set external fuel volume for each payload station , called for weapon init and on reload
	*/
	FM_API void ed_fm_set_external_fuel(int	 station,
														double fuel,
														double x,
														double y,
														double z);
	...
	*/
	FM_API double ed_fm_get_external_fuel();
	...
	*/
	FM_API void ed_fm_refueling_add_fuel(double fuel);
	...
	*/
	FM_API void ed_fm_set_draw_args (EdDrawArgument * drawargs,size_t size);
	FM_API void ed_fm_set_fc3_cockpit_draw_args (EdDrawArgument * drawargs,size_t size);

	/*
	shake level amplitude for head simulation , 
	*/
	FM_API double ed_fm_get_shake_amplitude();

	/*
	will be called for your internal configuration
	*/
	FM_API void ed_fm_configure(const char * cfg_path);

	/*
	will be called for your internal configuration
	void ed_fm_release   called when fm not needed anymore : aircraft death etc.
	*/
	FM_API void ed_fm_release ();

	/*
	various param call back to proper integrate your FM to DCS , like engine RPM , thrust etc
	*/
	FM_API double ed_fm_get_param(unsigned param_enum);
	...
	*/
	FM_API void ed_fm_cold_start();
	FM_API void ed_fm_hot_start();
	FM_API void ed_fm_hot_start_in_air();
	...
	*/
	FM_API bool ed_fm_make_balance (double & ax,//linear acceleration component in world coordinate system);
									  double & ay,//linear acceleration component in world coordinate system
									  double & az,//linear acceleration component in world coordinate system
									  double & vx,//linear velocity component in world coordinate system
									  double & vy,//linear velocity component in world coordinate system
	...
	*/
	FM_API bool ed_fm_enable_debug_info();
	...
	FM_API size_t ed_fm_debug_watch(int level, char *buffer,size_t maxlen);

	/* 
	path to your plugin installed
	*/
	FM_API void ed_fm_set_plugin_data_install_path(const char * path);

	// damages and failures
	// callback when preplaneed failure triggered from mission 
	FM_API void ed_fm_on_planned_failure(const char * data);

	// callback when damage occurs for airframe element 
	FM_API void ed_fm_on_damage(int Element, double element_integrity_factor);

	// called in case of repair routine 
	FM_API void ed_fm_repair();

	// in case of some internal damages or system failures this function return true , to switch on repair process
	FM_API bool ed_fm_need_to_be_repaired();

	// inform about  invulnerability settings
	FM_API void ed_fm_set_immortal(bool value);
	// inform about  unlimited fuel
	FM_API void ed_fm_unlimited_fuel(bool value);
	// inform about simplified flight model request 
	FM_API void ed_fm_set_easy_flight(bool value);

	// custom properties sheet 
	FM_API void ed_fm_set_property_numeric(const char * property_name,float value);
	// custom properties sheet 
	FM_API void ed_fm_set_property_string(const char * property_name,const char * value);
	...
	*/
	FM_API bool ed_fm_pop_simulation_event(ed_fm_simulation_event & out);

	/*
		feedback to your fm about suspension state
	*/
	FM_API void ed_fm_suspension_feedback(int idx,const ed_fm_suspension_info * info);
